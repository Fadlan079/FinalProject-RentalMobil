...
class Mobil{
    ...
    public function countFilterMobil($harga = null, $transmisi = null, $bhn_bkr = null, $kursi = null) {    try {        $sql = "SELECT COUNT(DISTINCT mobil.id_mobil) AS total                FROM mobil                JOIN tipemobil ON mobil.id_tipe = tipemobil.id_tipe                WHERE 1=1";        $params = [];        // 🔧 SAMA SEPERTI DI filterMobil()        if (!empty($harga) && $harga != 'semua') {            if ($harga === 'lt1jt') {                $sql .= " AND tipemobil.harga < 1000000";            } elseif ($harga === 'lt5jt') {                $sql .= " AND tipemobil.harga < 5000000";            } elseif ($harga === 'gt5jt') {                $sql .= " AND tipemobil.harga > 5000000";            }        }        if (!empty($transmisi) && $transmisi != 'semua') {            $sql .= " AND tipemobil.transmisi = :transmisi";            $params[':transmisi'] = $transmisi;        }        if (!empty($bhn_bkr) && $bhn_bkr != 'semua') {            $sql .= " AND tipemobil.bhn_bkr = :bhn_bkr";            $params[':bhn_bkr'] = $bhn_bkr;        }        if (!empty($kursi) && $kursi != 'semua') {            if ($kursi === '2') {                $sql .= " AND tipemobil.kursi = 2";            }             elseif ($kursi === '4-5') {                $sql .= " AND tipemobil.kursi IN (4, 5)";            }            elseif ($kursi === '6-8') {                $sql .= " AND tipemobil.kursi IN (6, 7, 8)";            }        }        $stmt = $this->pdo->prepare($sql);                // Binding parameters        foreach ($params as $key => $value) {            $stmt->bindValue($key, $value);        }                $stmt->execute();        return $stmt->fetchColumn();    } catch(PDOException $e) {        echo "Gagal Menghitung Filter: " . $e->getMessage();        return 0;    }}
