...
class PELANGGANController{
   ...
   public function index() {
    ...
    // 🔧 PASTIKAN parameter ini sesuai
    $harga = $_GET['harga'] ?? '';
    $transmisi = $_GET['transmisi'] ?? '';
    $bhn_bkr = $_GET['bhn_bkr'] ?? '';
    $kursi = $_GET['kursi'] ?? '';
    
    // 🔧 PERBAIKI logic hasFilter
    $hasFilter = ($harga !== '' && $harga !== 'semua') || 
                 ($transmisi !== '' && $transmisi !== 'semua') || 
                 ($bhn_bkr !== '' && $bhn_bkr !== 'semua') || 
                 ($kursi !== '' && $kursi !== 'semua');
    ...
    if (!empty($query)) {
        ...
        // 🧩 Filter mobil - PASTIKAN parameter dikirim dengan benar
