                ...
                <div class="bg-neutral-900  shadow-md rounded-xl p-4 flex flex-col md:flex-row items-center justify-between border-t-5 border-orange-500 hover:shadow-lg transition-shadow duration-300">
                    <div class="flex items-center space-x-4">
                        ...
                        <div>
                            ...
                            <p class="text-sm text-neutral-400"><span>Model / Tipe :</span> <?= htmlspecialchars($tr['model'] ?? '-') ?> / <?= htmlspecialchars($tr['tipe'] ?? '-') ?></p>
                            <p class="text-sm text-neutral-400"><span>No Plat :</span> <?= htmlspecialchars($tr['noplat'] ?? '-') ?></p>
                            <p class="text-sm text-neutral-400"><span>Durasi :</span> <?= $tr['durasi_sewa'] ?> hari</p>
                            <p class="text-sm text-neutral-400"><span>Harga / Hari :</span>Rp <?= number_format($harga_per_hari,2) ?></p>
                            <p class="text-sm text-neutral-400"><span>Tanggal Sewa :</span> <?= date('d M Y', strtotime($tr['tgl_sewa'])) ?></p>
                            <p class="text-sm text-neutral-400"><span>Tanggal Kembali :</span> <?= date('d M Y', strtotime($tr['tgl_kembali'])) ?></p>
                            <p class="text-sm text-neutral-400"><span>Pegawai :</span><?= htmlspecialchars($tr['nama_pegawai'] ?? 'Belum ditugaskan') ?></p>
                            <p class="text-sm mt-1 text-neutral-600">Status: <span class="font-semibold text-yellow-600"><?= ucfirst($tr['status']) ?></span></p>
                            <?php if ($sisa_hari >= 0): ?>
                    ...
                    <div class="mt-4 md:mt-0 flex flex-col space-y-2">
                        ...
                        <button onclick="editPesanan(<?= $tr['id_transaksi'] ?>)" 
                                class="bg-orange-500 text-white px-4 py-1 rounded-md hover:bg-orange-600 transition-colors">
                            Edit
                        </button>
                    </div>
                </div>
