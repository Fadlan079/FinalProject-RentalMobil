            ...
            <div class="z-10 relative">
                ...
                <div class="p-2 m-2 grid grid-cols-3 gap-2 text-xs">
                    ...
                    <button 
                        ...
                        <?= in_array($row['status'], ['rent', 'maintenance']) ? 'opacity-50 cursor-not-allowed' : '' ?>"
                        data-id="<?= $row['id_mobil'] ?>"
                        <?= in_array($row['status'], ['rent', 'maintenance']) ? 'disabled' : '' ?>
                    >
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Handle sewa button clicks
    document.querySelectorAll('.sewa-btn').forEach(button => {
        button.addEventListener('click', function() {
            if (!this.disabled) {
                const idMobil = this.getAttribute('data-id');
                // Tambahkan logika untuk membuka modal sewa
                console.log('Sewa mobil ID:', idMobil);
            }
        });
    });
});
