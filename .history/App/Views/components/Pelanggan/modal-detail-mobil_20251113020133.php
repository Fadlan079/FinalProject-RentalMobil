...
<div id="modalDetail" class="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm hidden p-4 pointer-events-none">
  <div class="relative bg-neutral-900 text-neutral-100 rounded-2xl shadow-2xl w-full max-w-3xl p-6 border border-neutral-800 max-h-[90vh] overflow-y-auto pointer-events-auto">
...
<script>
  ...
  function closeModal() {
    ...
    modal.classList.add('pointer-events-none'); // supaya overlay tidak blok klik
  ...
  // Close modal ketika tombol X diklik
  ...
  // Fungsi buka modal + load data mobil
    ...
    fetch(`index.php?action=detail-mobil&id=${id}`)
      ...
      .then(data => {
        ...
        form.addEventListener('submit', async e => {
          ...
          try {
            const res = await fetch('index.php?action=store-transaksi', {
              method: 'POST',
            ...
            if (resp.success) {
              alert(resp.success);
              closeModal(); // reset modal & enable klik kembali
            } else {
