...
<form action="index.php" method="GET" class="max-w-6xl mx-auto flex flex-col lg:flex-row justify-between items-stretch lg:gap-3">
    ...
    <div id="typebar" class="fixed z-50 left-0 top-0 w-full sm:w-[400px] p-8 space-y-6 bg-neutral-900 h-full overflow-y-auto scrollbar-hide transform -translate-x-full transition-transform duration-300 ease-in-out">
...
$bhn_bkr = isset($_GET['bhn_bkr']) ? trim($_GET['bhn_bkr']) : ''; // Diperbaiki: $bahan_bkr menjadi $bhn_bkr
$kursi = isset($_GET['kursi']) ? trim($_GET['kursi']) : ''; // Diperbaiki: $kapasitas menjadi $kursi
?>
    ...
    <?php for ($i = 1; $i <= $totalPages; $i++): ?>
        <?php
            $queryParams = http_build_query([
                ...
                'harga' => $harga,
                'transmisi' => $transmisi,
                'bhn_bkr' => $bhn_bkr, // Konsisten dengan nama variabel
                'kursi' => $kursi // Konsisten dengan nama variabel
            ]);
        ?>
        <a href="?<?= $queryParams ?>#daftar-mobil"
...
<!-- JavaScript untuk Modal dan Filter Mobile -->
<script>
