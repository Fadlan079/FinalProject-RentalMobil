<!-- MODAL DETAIL MOBIL -->
<div id="modalDetail" class="fixed inset-0 z-50 hidden bg-black/70 backdrop-blur-sm flex items-start justify-center overflow-auto p-4">
  <div class="relative bg-neutral-900 text-neutral-100 rounded-2xl shadow-2xl w-full max-w-3xl p-6 border border-neutral-800 max-h-[90vh] overflow-y-auto mt-10">
    ...
    .then(data => {
      ...
      modalContent.innerHTML = `
        <div class="flex flex-col md:flex-row gap-6 border-b border-neutral-700 pb-4 mb-5">
          ...
          <div class="flex-1 flex flex-col justify-between">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-sm text-neutral-400">
              ...
              <div><span class="font-medium text-neutral-200 text-orange-500 font-semibold">Harga:</span> Rp ${hargaPerHari.toLocaleString()}/hari</div>
            </div>
          </div>
        </div>
        ...
        <form id="formTransaksi" class="space-y-5">
          ...
          <input type="hidden" name="id_pegawai" value="5">
      ...
      form.addEventListener('submit', async e => {
        ...
        try {
          const res = await fetch('index.php?action=store-transaksi', { method: 'POST', body: new FormData(form) });
          const resp = await res.json();

          if (resp.success) {
            alert(resp.success);

            // --- UPDATE STATUS DI LIST MOBIL ---
            const btn = document.querySelector(`.btn-detail[data-id="${data.id_mobil}"]`);
            if (btn) {
if (btn) {
  btn.disabled = true;
  btn.classList.remove('bg-orange-500', 'hover:bg-orange-600');
  btn.classList.add('bg-red-500', 'cursor-not-allowed', 'pointer-events-none', 'opacity-50');
  btn.innerText = 'Sedang Disewa';
}

              
              const statusTag = btn.closest('div.grid').querySelector(`div.absolute.top-2.right-2`);
              if (statusTag) {
                statusTag.innerText = 'Rent';
                statusTag.classList.remove('bg-emerald-500', 'text-emerald-50', 'bg-yellow-500', 'text-yellow-50');
                statusTag.classList.add('bg-red-500', 'text-red-50');
              }

              // --- REORDER GRID OTOMATIS ---
              reorderMobilGrid();
            }
            closeModal();
          } else {
...
function reorderMobilGrid() {
  const grid = document.querySelector('.grid.grid-cols-1'); // ubah selector sesuai container grid mobil
  if (!grid) return;

  const cards = Array.from(grid.children);
  const orderMap = { 'Ready': 1, 'Rent': 2, 'Maintenance': 3 };

  cards.sort((a, b) => {
    const statusA = a.querySelector('div.absolute.top-2.right-2')?.innerText || '';
    const statusB = b.querySelector('div.absolute.top-2.right-2')?.innerText || '';
