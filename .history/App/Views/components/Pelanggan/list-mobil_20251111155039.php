...
<form action="index.php" method="GET" class="max-w-6xl mx-auto flex flex-col lg:flex-row justify-between items-stretch lg:gap-3">
    ...
    <div class="hidden lg:flex justify-between items-center bg-neutral-800 text-orange-50 p-2 rounded-xl shadow-md m-auto text-sm">
        ...
        <select name="harga" class="bg-neutral-800 p-1 rounded-lg focus:ring-2 focus:ring-orange-500">
            <option value="semua">Harga (Semua)</option>
            <option value="lt1jt">Kurang dari 1 juta</option>
            <option value="lt5jt">Kurang dari 5 juta</option>
        </select>

        <select name="transmisi" class="bg-neutral-800 p-1 rounded-lg focus:ring-2 focus:ring-orange-500">
            <option value="semua">Transmisi (Semua)</option>
            <option value="automatic">Automatic</option>
            <option value="manual">Manual</option>
        </select>

        <select name="bhn_bkr" class="bg-neutral-800 p-1 rounded-lg focus:ring-2 focus:ring-orange-500">
            <option value="semua">Bahan Bakar (Semua)</option>
            <option value="listrik">Listrik</option>
            <option value="bensin">Bensin</option>
        </select>

        <select name="kapasitas" class="bg-neutral-800 p-1 rounded-lg focus:ring-2 focus:ring-orange-500">
            <option value="semua">Kapasitas (Semua)</option>
    ...
    <div id="typebar" class="fixed z-200 left-0 top-0 w-full p-8 space-y-6 bg-neutral-900 h-full overflow-y-auto scrollbar-hide transform -translate-x-full transition-all duration-300">
        ...
        <!-- <div class="flex flex-col gap-3">
            ...
            <label for="">
            <input type="radio" name="kapasitas" value="semua">
            Semua
            </label>
            <label for="">
            <input type="radio" name="kapasitas" value="2">
            2 Orang
            </label>
            <label for="">
            <input type="radio" name="kapasitas" value="4-5">
            4-5 Orang
            </label>
            <label for="">
            <input type="radio" name="kapasitas" value="6-8">
            6-8 Orang
            </label>              
        </div> -->
    ...
    <?php if(!empty($data)):?>
        ...
        <div class="bg-neutral-800 border-t-4 border-orange-500 text-neutral-300 shadow-xl group relative tracking-wide">
            ...
            <div class="p-2 m-2 grid grid-cols-3 gap-2 text-xs">
                ...
                <a href="index.php?action=detail-mobil&id=<?=$row['id_mobil']?>" class="col-span-3 p-2 bg-orange-500 rounded-xl shadow-lg font-semibold text-orange-50 text-center hover:bg-orange-600 transition-all duration-300">Sewa Sekarang</a>
            </div>
            </div>
        </div>
        <?php endforeach?>
