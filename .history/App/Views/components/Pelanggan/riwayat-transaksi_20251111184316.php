...
<?php if(empty($riwayat)): ?>
    <div class="flex flex-col items-center justify-center min-h-[60vh] text-center px-4 space-y-5">
        ...
        <p class="text-neutral-400 text-base md:text-lg max-w-md leading-relaxed">
            Riwayat transaksi akan muncul di sini setelah kamu melakukan penyewaan mobil.
        </p>
        ...
        <!-- 🔍 DEBUG INFO -->
        <?php if(!empty($transaksi_simple)): ?>
            <div class="mt-4 p-4 bg-yellow-100 border border-yellow-400 rounded">
                <p class="text-sm text-yellow-800">
                    <strong>Debug:</strong> Ada <?= count($transaksi_simple) ?> transaksi di database tapi tidak tampil di riwayat. 
                    Masalah mungkin di query JOIN.
                </p>
            </div>
        <?php endif; ?>
    </div>
<?php else: ?>
    <?php foreach($riwayat as $tr): ?>
        ...
        // Fallback untuk gambar - gunakan mobil_img jika ada
        $gambar_mobil = !empty($tr['mobil_img']) ? $tr['mobil_img'] : ($tr['img'] ?? 'default.svg');
        ?>
        ...
        <div class="flex flex-col lg:flex-row bg-neutral-900 text-neutral-100 shadow-lg rounded-xl mb-4 overflow-hidden border-l-4 border-orange-500 hover:shadow-2xl transition-shadow duration-300">
            ...
            <div class="p-4 flex-1">
                <h2 class="font-semibold text-lg text-orange-500">
                    <?= htmlspecialchars($tr['merk'] ?? 'Mobil') ?> 
                    (<?= htmlspecialchars($tr['tahun'] ?? '-') ?>)
                </h2>
                ...
                <!-- 🔍 DEBUG INFO -->
                <p class="text-xs text-gray-500 mt-1">ID Transaksi: <?= $tr['id_transaksi'] ?></p>
            </div>
