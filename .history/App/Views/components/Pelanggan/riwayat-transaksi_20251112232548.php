      ...
      <?php else: ?>
        ...
        <div class="grid gap-6 md:grid-cols-2">
          <?php foreach($riwayat as $tr): ?>
            ...
            <div class="bg-neutral-900 text-neutral-100 shadow-lg rounded-xl overflow-hidden border-l-4 border-orange-500 hover:shadow-2xl transition-shadow duration-300 flex flex-col lg:flex-row">
              <img src="uploads/<?= htmlspecialchars($gambar_mobil) ?>" 
                   alt="Gambar <?= htmlspecialchars($tr['merk'] ?? 'Mobil') ?>" 
                   class="w-full lg:w-48 object-cover h-48 lg:h-auto"
              ...
              <div class="p-4 flex-1 flex flex-col justify-between">
                <div>
                  ...
                  <p class="text-sm text-neutral-400 mt-1">Warna: <span class="capitalize"><?= htmlspecialchars($tr['warna']) ?></span></p>
                  <p class="text-sm text-neutral-400">Nomor Plat: <?= htmlspecialchars($tr['noplat']) ?></p>
                  <p class="text-sm text-neutral-400">Durasi: <?= $tr['durasi_sewa'] ?> hari</p>
                  <p class="text-sm text-neutral-400">Harga / Hari: <span class="text-orange-500 font-medium">Rp <?= number_format($harga_per_hari,2) ?></span></p>
                  <p class="text-sm text-neutral-400">Tanggal Sewa: <?= date('d M Y', strtotime($tr['tgl_sewa'])) ?></p>
                  <p class="text-sm text-neutral-400">Tanggal Kembali: <?= date('d M Y', strtotime($tr['tgl_kembali'])) ?></p>
                  <p class="text-sm text-neutral-400">Pegawai: <?= htmlspecialchars($tr['nama_pegawai'] ?? '-') ?></p>
                ...
                <div class="mt-2 text-right">
                  ...
                  <p class="text-xs text-neutral-500">Transaksi dibuat: <?= date('d M Y H:i', strtotime($tr['tgl_dibuat'])) ?></p>
                </div>
              </div>
            </div>
