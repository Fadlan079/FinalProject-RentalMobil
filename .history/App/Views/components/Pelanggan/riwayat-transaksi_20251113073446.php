            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div class="bg-neutral-900 shadow-sm rounded-xl p-4 border-t-5 border-orange-500 hover:shadow-md transition-shadow duration-300">
                    <div class="flex items-center space-x-4">
                        ...
                        <div class="flex-1">
                            <h2 class="text-lg font-semibold"><?= htmlspecialchars($tr['merk'] ?? '-') ?> (<?= $tr['tahun'] ?>)</h2>
                            <p class="text-sm text-neutral-300"><span class="text-orange-400">Model / Tipe :</span> <?= htmlspecialchars($tr['model'] ?? '-') ?> / <?= htmlspecialchars($tr['tipe'] ?? '-') ?></p>
                            <p class="text-sm text-neutral-300"><span class="text-orange-400">No Plat :</span> <?= htmlspecialchars($tr['noplat'] ?? '-') ?></p>
                            <p class="text-sm text-neutral-300"><span class="text-orange-400">Durasi :</span> <?= $tr['durasi_sewa'] ?> hari</p>
                            <p class="text-sm text-neutral-300"><span class="text-orange-400">Tanggal Sewa :</span> <?= date('d M Y', strtotime($tr['tgl_sewa'])) ?></p>
                            <p class="text-sm text-neutral-300"><span class="text-orange-400">Tanggal Kembali :</span> <?= date('d M Y', strtotime($tr['tgl_kembali'])) ?></p>
                            <p class="text-sm text-neutral-300"><span class="text-orange-400">Pegawai :</span> <?= htmlspecialchars($tr['nama_pegawai'] ?? 'Belum ditugaskan') ?></p>
