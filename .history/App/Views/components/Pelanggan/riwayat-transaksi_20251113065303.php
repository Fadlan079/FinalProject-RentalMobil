...
// letakkan di paling atas (setelah session/require)
$projectRoot = dirname(__DIR__, 4); // dari App/Views/components/pelanggan -> naik 4 = root project
$uploadsDir  = $projectRoot . '/public/uploads/'; // path di filesystem
$uploadsUrl  = '/uploads/'; // URL yang dipakai browser (karena public/ adalah web root)
...
$gambar_file = $tr['img'] ?? '';
$gambar_path = $uploadsDir . $gambar_file; // filesystem
$gambar = (!empty($gambar_file) && file_exists($gambar_path)) ? $gambar_file : $defaultImg;

                    if (!empty($gambar_file) && file_exists($gambar_path)) {
                        $gambar = $gambar_file;
                    } else {
                ...
                <div class="bg-white shadow-md rounded-xl p-4 flex flex-col md:flex-row items-center justify-between border border-gray-100 hover:shadow-lg transition-shadow duration-300">
                    <div class="flex items-center space-x-4">
                        <img src="/uploads/<?= htmlspecialchars($gambar) ?>"  <img src="<?= $uploadsUrl . htmlspecialchars($gambar) ?>"     class="w-24 h-24 object-cover rounded-lg" alt="Mobil">
                        <div>
        ...
        <?php else: ?>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <?php foreach($riwayatSelesai as $tr):
                    ...
                    $gambar_path = __DIR__ . '/../../../../Public/assets/uploads/' . ($tr['img'] ?? 'default.jpg');
                ...
                <div class="bg-gray-50 shadow-sm rounded-xl p-4 border border-gray-100 hover:shadow-md transition-shadow duration-300">
                    <div class="flex items-center space-x-4">
                        <img src="../Public/assets/uploads/<?= htmlspecialchars($gambar) ?>" 
                             onerror="this.src='../Public/assets/uploads/default.jpg'"
